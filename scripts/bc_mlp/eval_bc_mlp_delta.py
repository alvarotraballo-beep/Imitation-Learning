#!/usr/bin/env python3
"""Evaluate the robust delta BC-MLP in MuJoCo."""
...
from train_bc_mlp_delta import DeltaBCMLP  # noqa: E402
...
def obs_vector(env, obs_keys, progress, cube_initial_pos, cube_half_size, cube_rgba):
    ...
    obs["cube_color"] = np.asarray(cube_rgba, dtype=np.float32)
    obs["pad_mid_pos"] = get_pad_mid_pos(env).astype(np.float32)
    return np.concatenate([np.asarray(obs[k], dtype=np.float32).reshape(-1) for k in obs_keys], axis=0).astype(np.float32)
...
def load_policy(path, device):
    ...
    tensors = {
        ...
        "delta_mean": torch.from_numpy(ckpt["delta_mean"].astype(np.float32)).to(device),
        "delta_std": torch.from_numpy(ckpt["delta_std"].astype(np.float32)).to(device),
    }
...
def run_rollout(args, ckpt, model, tensors, rollout_idx, scenario, video_path=None):
    ...
    cube = np.asarray(scenario.get("cube", args.cube), dtype=np.float64)
    cube_half_size = float(scenario.get("cube_half_size", args.cube_half_size))
    cube_rgba = np.asarray(scenario.get("cube_rgba", args.cube_rgba), dtype=np.float64)
    initial_q = np.asarray(scenario.get("initial_q", args.initial_q), dtype=np.float64)
    if not args.keep_cube_z:
        cube = cube.copy()
        cube[2] += cube_half_size - args.reference_cube_half_size
    ...
    for step in range(args.horizon):
        ...
        x_np = obs_vector(env, ckpt["obs_keys"], progress, cube_initial_pos, cube_half_size, cube_rgba)[None]
        x = torch.from_numpy(x_np).to(device)
        with torch.no_grad():
            x_norm = (x - tensors["obs_mean"]) / tensors["obs_std"]
            pred_delta_norm, pred_grip_logit = model(x_norm)
            delta = (pred_delta_norm * tensors["delta_std"] + tensors["delta_mean"]).detach().cpu().numpy()[0]
            close_prob = float(torch.sigmoid(pred_grip_logit).detach().cpu().numpy()[0])
        delta = np.clip(delta, -args.max_joint_step, args.max_joint_step)
        if args.command_smoothing > 0.0:
            alpha = float(np.clip(args.command_smoothing, 0.0, 0.98))
            delta = alpha * prev_delta + (1.0 - alpha) * delta
        prev_delta = delta.copy()
        q_cmd = env.sim.data.qpos[:6].copy() + delta
        ...
        if step % args.print_every == 0 or step == args.horizon - 1:
            print(
                f"rollout={rollout_idx} step={step:04d} progress={progress:.3f} close_prob={close_prob:.3f} "
                f"close={close} cube={np.round(cube_geom_pos(env), 4)} "
                f"delta={np.round(delta, 4)} bad_table_contacts={bad_contacts}"
            )
...
def main():
    ...
    parser.add_argument("--checkpoint", default="bc_mlp_policy_experiment/robust_v2/models/bc_mlp_delta_robust.pt")
    parser.add_argument("--scenario-file", default=None)
    parser.add_argument("--rollouts", type=int, default=1)
    parser.add_argument("--horizon", type=int, default=2954)
    ...
    for rollout_idx, scenario in enumerate(scenarios[: args.rollouts]):
        ...
        result = run_rollout(args, ckpt, model, tensors, rollout_idx, scenario, video_path=rollout_video)
        results.append(result)
        print(
            f"rollout={rollout_idx} success={result['success']} success_step={result['success_step']} "
            f"final_cube={np.round(result['final_cube'], 4)} max_bad_table_contacts={result['max_bad_table_contacts']}"
        )
        if result["last_bad_pairs"]:
            print("last_bad_table_pairs:", result["last_bad_pairs"])
    successes = sum(int(r["success"]) for r in results)
    print("robust bc_mlp successes:", successes, "/", len(results))
